...
#here we can get an understanding of what is in the csv. we see that it is not all numbers and instead has some symbols in it. this is why it is showing up as a character class
```
...
snowsurvey_fixed <- snowsurvey_csv %>% 
  #filter(Snow_cover == ".") %>%  #this was to test just for that portion of the dataset. because we commented it out, we now applied it to the whole dataset
